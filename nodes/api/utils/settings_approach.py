...
class QingConfigManager:
    ...
    def __init__(self):
        # 插件目录和配置文件路径 (现在在utils子目录中，需要回到上级api目录)
        self.dir_path = Path(__file__).parent.parent  # 从utils回到api目录
...
# 全局配置管理器实例（延迟初始化）
_qing_config_manager = None

def get_config_manager():
    """获取配置管理器实例（延迟初始化）"""
    global _qing_config_manager
    if _qing_config_manager is None:
        _qing_config_manager = QingConfigManager()
    return _qing_config_manager

# 为了兼容性，保留原来的名称（作为函数调用）
def qing_config_manager():
...
def get_qing_api_key_improved(config_key: str = "glm_api_key") -> str:
    ...
    Returns:
        str: API密钥
    """
    return get_config_manager().get_api_key(config_key)
...
def set_qing_api_key_improved(api_key: str, source: str = "unknown", config_key: str = "glm_api_key") -> bool:
    ...
    Returns:
        bool: 是否设置成功
    """
    return get_config_manager().update_api_key(api_key, source, config_key)